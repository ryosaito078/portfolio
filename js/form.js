jQuery(function($){
  ...
  function show_error(message, this$) {
    text = this$.parent().find('dt').text() + message;
  ...
  $("#main_contact").submit(function(){
    ...
    $(":text,textarea").filter(".validate").each(function(){
      ...
      $(this).filter(".required").each(function(){
        if($(this).val()==""){
          show_error(" 未入力です", $(this));
        }
      })
      //文字数の検証
      $(this).filter(".max30").each(function(){
        if($(this).val().length > 30){
          show_error(" こちらは30文字以内でお願いします", $(this));
        }
      })
      $(this).filter(".max50").each(function(){
        if($(this).val().length > 50){
          show_error(" こちらは50文字以内でお願いします", $(this));
        }
      })
      $(this).filter(".max100").each(function(){
        if($(this).val().length > 100){
          show_error(" こちらは100文字以内でお願いします", $(this));
        }
      })
      $(this).filter(".max1000").each(function(){
        if($(this).val().length > 1000){
          show_error(" こちらはは1000文字以内でお願いします", $(this));
        }
      })
    })
